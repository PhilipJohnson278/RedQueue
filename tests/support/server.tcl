...
proc spawn_server {config_file stdout stderr args} {
    set cmd [list src/redqueue-server $config_file]
    set args {*}$args
    if {[llength $args] > 0} {
        lappend cmd {*}$args
    }
...
proc start_server {options {code undefined}} {
    ...
    if {$::tls} {
        if {$::tls_module} {
            lappend config_lines [list "loadmodule" [format "%s/src/redqueue-tls.so" [pwd]]]
        }
    ...
    set config_file [tmpfile redqueue.conf]
