...
proc redisbenchmark {host port {opts {}}} {
    set cmd [list src/redqueue-benchmark -h $host -p $port]
...
proc redisbenchmarkuri {host port {opts {}}} {
    set cmd [list src/redqueue-benchmark -u redis://$host:$port]
    lappend cmd {*}[redisbenchmark_tls_config "tests"]
    lappend cmd {*}$opts
    return $cmd
}
...
proc redisbenchmarkuriuserpass {host port user pass {opts {}}} {
    set cmd [list src/redqueue-benchmark -u redis://$user:$pass@$host:$port]
    lappend cmd {*}[redisbenchmark_tls_config "tests"]
    lappend cmd {*}$opts
    return $cmd
}
