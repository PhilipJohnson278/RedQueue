...
# Returns command line for executing redqueue-cli
proc rediscli {host port {opts {}}} {
    set cmd [list src/redqueue-cli -h $host -p $port]
    lappend cmd {*}[rediscli_tls_config "tests"]
    lappend cmd {*}$opts
    return $cmd
}

# Returns command line for executing redqueue-cli with a unix socket address
proc rediscli_unixsocket {unixsocket {opts {}}} {
    return [list src/redqueue-cli -s $unixsocket {*}$opts]
}

# Run redqueue-cli with specified args on the server of specified level.
