start_server {tags {"introspection"}} {
    ...
    test {redqueue-server command line arguments - allow passing option name and option value in the same arg} {
    ...
    test {redqueue-server command line arguments - wrong usage that we support anyway} {
    ...
    test {redqueue-server command line arguments - allow option value to use the `--` prefix} {
    ...
    test {redqueue-server command line arguments - option name and option value in the same arg and `--` prefix} {
        start_server {config "default.conf" args {"--proc-title-template --my--title--template" "--loglevel verbose"}} {
            assert_match [r config get proc-title-template] {proc-title-template --my--title--template}
            assert_match [r config get loglevel] {loglevel verbose}
        }
    ...
    test {redqueue-server command line arguments - save with empty input} {
    ...
    test {redqueue-server command line arguments - take one bulk string with spaces for MULTI_ARG configs parsing} {
