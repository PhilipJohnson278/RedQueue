# Primitive tests on cluster-enabled redqueue using redqueue-cli
...
start_multiple_servers 3 [list overrides $base_conf] {
    ...
    test {Create 3 node cluster} {
        exec src/redqueue-cli --cluster-yes --cluster create \
    ...
    test "Perform a Resharding" {
        exec src/redqueue-cli --cluster-yes --cluster reshard 127.0.0.1:[srv -2 port] \
    ...
    test "Wait for cluster to be stable" {
        ...
        wait_for_condition 1000 50 {
            [catch {exec src/redqueue-cli --cluster check 127.0.0.1:[srv 0 port]}] == 0 &&
            [catch {exec src/redqueue-cli --cluster check 127.0.0.1:[srv -1 port]}] == 0 &&
            [catch {exec src/redqueue-cli --cluster check 127.0.0.1:[srv -2 port]}] == 0 &&
            [CI 0 cluster_state] eq {ok} &&
            [CI 1 cluster_state] eq {ok} &&
            [CI 2 cluster_state] eq {ok}
        } else {
    ...
    test "use previous hostip in \"cluster-preferred-endpoint-type unknown-endpoint\" mode" {
        ...
        # when redqueue-cli not in cluster mode, return MOVE with empty host
        ...
        assert_equal "[exec src/redqueue-cli -h 127.0.0.1 -p [srv 0 port] -c set foo bar]" {OK}
        assert_match "[exec src/redqueue-cli -h 127.0.0.1 -p [srv 0 port] -c get foo]" {bar}
...
# Test redqueue-cli -- cluster create, add-node, call.
...
start_multiple_servers 5 [list overrides $base_conf] {
    ...
    test {Functions are added to new node on redqueue-cli cluster add-node} {
        exec src/redqueue-cli --cluster-yes --cluster create \
                           127.0.0.1:[srv 0 port] \
                           127.0.0.1:[srv -1 port] \
                           127.0.0.1:[srv -2 port]
        ...
        exec src/redqueue-cli --cluster-yes --cluster call 127.0.0.1:[srv 0 port] \
                           FUNCTION LOAD {#!lua name=TEST
                               redis.register_function('test', function() return 'hello' end)
                           }

        # adding node to the cluster
        exec src/redqueue-cli --cluster-yes --cluster add-node \
...
# Test redqueue-cli --cluster create, add-node.
# Test that one slot can be migrated to and then away from the new node.
test {Migrate the last slot away from a node using redqueue-cli} {
    start_multiple_servers 4 [list overrides $base_conf] {
        ...
        # Insert some data
        assert_equal OK [exec src/redqueue-cli -c -p [srv 0 port] SET foo bar]
        set slot [exec src/redqueue-cli -c -p [srv 0 port] CLUSTER KEYSLOT foo]
        ...
        wait_for_condition 1000 50 {
            ...
            [catch {exec src/redqueue-cli --cluster check 127.0.0.1:[srv -1 port]}] == 0 &&
            [catch {exec src/redqueue-cli --cluster check 127.0.0.1:[srv -2 port]}] == 0 &&
            [catch {exec src/redqueue-cli --cluster check 127.0.0.1:[srv -3 port]}] == 0 &&
            [CI 0 cluster_state] eq {ok} &&
            [CI 1 cluster_state] eq {ok} &&
            [CI 2 cluster_state] eq {ok} &&
            [CI 3 cluster_state] eq {ok}
        ...
        # Move the only slot back to original node using redqueue-cli
        exec src/redqueue-cli --cluster reshard 127.0.0.1:[srv -3 port] \
...
# Test redqueue-cli --cluster create, add-node with cluster-port.
...
start_server [list overrides [list cluster-enabled yes cluster-node-timeout 1 cluster-port [find_available_port $::baseport $::portcount]]] {
    ...
    test {redqueue-cli --cluster create with cluster-port} {
        exec src/redqueue-cli --cluster-yes --cluster create \
                           127.0.0.1:[srv 0 port] \
                           127.0.0.1:[srv -1 port] \
                           127.0.0.1:[srv -2 port]
    ...
    test {redqueue-cli --cluster add-node with cluster-port} {
