...
start_server {
    ...
    test {XADD with MAXLEN option with expiration time} {
        r DEL mystream
        for {set j 0} {$j < 1000} {incr j} {
            if {rand() < 0.9} {
                r XADD mystream MAXLEN 5 EX 10 * xitem $j
            } else {
                r XADD mystream MAXLEN 5 PX 12345 * yitem $j
            }
        }
        assert {[r xlen mystream] == 5}
        set res [r xrange mystream - +]
    ...
    test {XADD with MAXLEN option and the '=' argument with expiration time} {
        r DEL mystream
        for {set j 0} {$j < 1000} {incr j} {
            if {rand() < 0.9} {
                r XADD mystream MAXLEN = 5 EX 10 * xitem $j
    ...
    test {XADD with MAXLEN option and the '~' argument with expiration time} {
        ...
        for {set j 0} {$j < 1000} {incr j} {
            if {rand() < 0.9} {
                r XADD mystream MAXLEN ~ 555 EX 10 * xitem $j
            } else {
                r XADD mystream MAXLEN ~ 555 PX 12345 * yitem $j
            }
        }
        assert {[r XLEN mystream] == 600}
    }
    ...
    test {XADD with NOMKSTREAM option and EX / PX option} {
        r DEL mystream
        assert_equal "" [r XADD mystream NOMKSTREAM EX 10 * item 1 value a]
        assert_equal 0 [r EXISTS mystream]
        r XADD mystream * item 1 value a
        r XADD mystream NOMKSTREAM PX 12345 * item 2 value b
        assert_equal 2 [r XLEN mystream]
        set items [r XRANGE mystream - +]
        assert_equal [lindex $items 0 1] {item 1 value a}
        assert_equal [lindex $items 1 1] {item 2 value b}
    ...
    test {XADD with MINID option and EX / PX option} {
        r DEL mystream
        for {set j 1} {$j < 1001} {incr j} {
            set minid 1000
            if {$j >= 5} {
            ...
            if {rand() < 0.9} {
                r XADD mystream MINID $minid EX 10 $j xitem $j
            } else {
                r XADD mystream MINID $minid PX 12345 $j yitem $j
            }
        }
        assert {[r xlen mystream] == 6}
        set res [r xrange mystream - +]
    ...
    test {XADD with LIMIT delete entries no more than limit with expiration time} {
        r del yourstream
        for {set j 0} {$j < 3} {incr j} {
            r XADD yourstream * xitem v
        }
        r XADD yourstream MAXLEN ~ 0 limit 1 EX 10 * xitem v
        assert {[r XLEN yourstream] == 4}
    }
    ...
    test {XREAD + multiple XADD inside transaction with expiration time} {
        r XADD s2 * old abcd1234
        set rd [redis_deferring_client]
        $rd XREAD BLOCK 20000 STREAMS s2 s2 s2 $ $ $
        wait_for_blocked_clients_count 1
        r MULTI
        r XADD s2 EX 10 * field one
        r XADD s2 PX 12345 * field two
        r XADD s2 * field three
        r EXEC
        set res [$rd read]
        assert {[lindex $res 0 0] eq {s2}}
    ...
    test {XDEL remove timeout too} {
        set id [r XADD somestream EX 10 * item 1 value a]
        set result [r XTTL somestream $id]
        set ttl [lindex $result 0 1]
        assert {$ttl <= 10000 && $ttl > 9900}
    ...
    # Here the idea is to check the consistency of the stream data structure
    # as we remove all the elements down to zero elements.
    test {XDEL fuzz test with expiration time} {
        r del somestream
        set ids {}
        set x 0; # Length of the stream
        while 1 {
            lappend ids [r xadd somestream EX 1000 * item $x]
            incr x
            # Add enough elements to have a few radix tree nodes inside the stream.
            if {[dict get [r xinfo stream somestream] radix-tree-keys] > 20} break
        }
    ...
    test {XADD with LIMIT consecutive calls and expiration time} {
        r del mystream
        r config set stream-node-max-entries 10
        for {set j 0} {$j < 100} {incr j} {
            r XADD mystream * xitem v
        }
        r XADD mystream MAXLEN ~ 55 LIMIT 30 EX 100 * xitem v
        assert {[r xlen mystream] == 71}
        r XADD mystream MAXLEN ~ 55 LIMIT 30 PX 12345 * xitem v
        assert {[r xlen mystream] == 62}
        r config set stream-node-max-entries 100
    }
    ...
    test {XADD with expiration time in seconds} {
        r DEL mystream
        r XADD mystream EX 1 123-0 item 1 value a
        set result [r XTTL mystream 123-0]
        set ttl [lindex $result 0 1]
        assert {$ttl <= 1000 && $ttl > 900}
        after 1100
        assert {[r xlen mystream] == 0}
    }

    test {XADD with expiration time in milliseconds} {
        r DEL mystream
        r XADD mystream PX 1000 123-0 item 1 value a
        set result [r XTTL mystream 123-0]
        set ttl [lindex $result 0 1]
        assert {$ttl <= 1000 && $ttl > 900}
        after 1100
    ...
    test {XDELAY append item with delay time in seconds} {
        r DEL mystream
        r XDELAY mystream SX 2 * item 1 value a
        assert {[r xlen mystream] == 0}
        after 2100
        assert {[r xlen mystream] == 1}
    }

    test {XDELAY append multiple items with delay time in seconds} {
        r DEL mystream
        r XDELAY mystream SX 1 + item 1 value a
        r XDELAY mystream MX 1234 + item 2 value a
        r XDELAY mystream SX 1 + item 3 value a
        assert {[r xlen mystream] == 0}
        after 1400
        assert {[r xlen mystream] == 3}

        r XDELAY mystream MX 1234 + item 4 value a
        r XDELAY mystream SX 1 + item 5 value a
        r XDELAY mystream MX 1234 + item 6 value a
        after 1400
        assert {[r xlen mystream] == 6}
    }

    test {XDELAY NOMKSTREAM append item with delay time in seconds} {
        r DEL mystream
        r XDELAY mystream NOMKSTREAM SX 2 * item 1 value a
        assert {[r xlen mystream] == 0}
        after 2100
        assert {[r xlen mystream] == 0}
    }

    test {XDELAY append item with delay time in milliseconds} {
        r DEL mystream
        r XDELAY mystream MX 2000 * item 1 value a
        assert {[r xlen mystream] == 0}
        after 2100
        assert {[r xlen mystream] == 1}
    }

    test {XDELAY NOMKSTREAM append item with delay time in milliseconds} {
        r DEL mystream
        r XDELAY mystream NOMKSTREAM MX 2000 * item 1 value a
        assert {[r xlen mystream] == 0}
        after 2100
        assert {[r xlen mystream] == 0}
    }
    
    test {XDELAY append item with delay time in seconds and expiration time in seconds} {
        r DEL mystream
        r XDELAY mystream SX 2 EX 1 * item 1 value a
        assert {[r xlen mystream] == 0}
        after 2100
        assert {[r xlen mystream] == 1}
        set data [r XRANGE mystream - +]
        set id [lindex $data 0 0]
        set result [r XTTL mystream $id]
        set ttl [lindex $result 0 1]
        assert {$ttl <= 1000 && $ttl > 800}
        after 1100
        assert {[r xlen mystream] == 0}
    }

    test {XDELAY NOMKSTREAM append item with delay time in seconds and expiration time in seconds} {
        r DEL mystream
        r XDELAY mystream NOMKSTREAM SX 2 EX 1 + item 1 value a
        assert {[r xlen mystream] == 0}
        after 2100
        assert {[r xlen mystream] == 0}
    }
    
    test {XDELAY append item with delay time in seconds and expiration time in milliseconds} {
        r DEL mystream
        r XDELAY mystream PX 1000 SX 2 * item 1 value a
        assert {[r xlen mystream] == 0}
        after 2100
        assert {[r xlen mystream] == 1}
        set data [r XRANGE mystream - +]
    ...
    test {XDELAY NOMKSTREAM append item with delay time in seconds and expiration time in milliseconds} {
        r DEL mystream
        r XDELAY mystream NOMKSTREAM PX 1000 SX 2 + item 1 value a
        assert {[r xlen mystream] == 0}
        after 2100
        assert {[r xlen mystream] == 0}
    }

    test {XDELAY append item with delay time in milliseconds and expiration time in seconds} {
        r DEL mystream
        r XDELAY mystream MX 2000 EX 1 * item 1 value a
        assert {[r xlen mystream] == 0}
        after 2100
        assert {[r xlen mystream] == 1}
        set data [r XRANGE mystream - +]
    ...
    test {XDELAY NOMKSTREAM append item with delay time in milliseconds and expiration time in seconds} {
        r DEL mystream
        r XDELAY mystream NOMKSTREAM MX 2000 EX 1 * item 1 value a
        assert {[r xlen mystream] == 0}
        after 2100
        assert {[r xlen mystream] == 0}
    }

    test {XDELAY append item with delay time in milliseconds and expiration time in milliseconds} {
        r DEL mystream
        r XDELAY mystream MX 2000 PX 1000 + item 1 value a
        assert {[r xlen mystream] == 0}
        after 2100
        assert {[r xlen mystream] == 1}
        set data [r XRANGE mystream - +]
    ...
    test {XDELAY NOMKSTREAM append item with delay time in milliseconds and expiration time in milliseconds} {
        r DEL mystream
        r XDELAY mystream NOMKSTREAM MX 2000 PX 1000 + item 1 value a
        assert {[r xlen mystream] == 0}
        after 2100
        assert {[r xlen mystream] == 0}
    }

    test {XEXPIRE item with expiration time in seconds} {
        r DEL mystream
        r XADD mystream 123-0 item 1 value a
        set result [r XTTL mystream 123-0]
        set ttl [lindex $result 0 1]
        assert {$ttl == -1}
        set count [r XEXPIRE mystream EX 1 123-0 123-1 123-2]
        assert {$count == 1}

        set result [r XTTL mystream 123-0 123-1 123-2]
        set ttl [lindex $result 0 1]
        assert {$ttl <= 1000 && $ttl > 900}
    }

    test {XEXPIRE item with expiration time in milliseconds} {
        r DEL mystream
        r XADD mystream 123-0 item 1 value a
        set result [r XTTL mystream 123-0]
        set ttl [lindex $result 0 1]
        ...
        set count [r XEXPIRE mystream PX 1000 123-0 123-1 123-2]
        assert {$count == 1}
        ...
        set count [r XEXPIRE mystream EX 10 123-0 123-1 123-2]
        assert {$count == 1}
        ...
        set cont [r XPERSIST mystream 123-0 123-1 123-2]
        assert {$count == 1}
    ...
    test {XEXPIRE multiple items with expiration time in seconds} {
        r DEL mystream
        r XADD mystream 123-0 item 1 value a
        r XADD mystream 123-1 item 2 value b
        r XADD mystream 123-2 item 3 value c
        r XADD mystream 123-3 item 4 value d
        set result [r XTTL mystream 123-0 123-1 123-2 123-3 123-4]
        assert {[llength $result] == 4}
        set ttl1 [lindex $result 0 1]
        set ttl2 [lindex $result 1 1]
        set ttl3 [lindex $result 2 1]
        set ttl4 [lindex $result 3 1]
        assert {$ttl1 == -1 && $ttl2 == -1 && $ttl3 == -1 && $ttl4 == -1}

        set count [r XEXPIRE mystream EX 1 123-0 123-1 123-2 123-4]
        assert {$count == 3}
        ...
        set ttl3 [lindex $result 2 1]
        set ttl4 [lindex $result 3 1]
        assert {$ttl1 <= 1000 && $ttl1 > 900}
        assert {$ttl2 <= 1000 && $ttl2 > 900}
        assert {$ttl3 <= 1000 && $ttl3 > 900}
        assert {$ttl4 == -1}
    }

    test {XEXPIRE multiple items with expiration time in milliseconds} {
        r DEL mystream
        r XADD mystream 123-0 item 1 value a
        r XADD mystream 123-1 item 2 value b
        r XADD mystream 123-2 item 3 value c
        ...
        set count [r XEXPIRE mystream PX 1000 123-0 123-1 123-2 123-4]
        assert {$count == 3}
    ...
    test {XPERSIST remove item's expiration time} {
        r DEL mystream
        r XADD mystream 123-0 item 1 value a
        set result [r XTTL mystream 123-0]
        set ttl [lindex $result 0 1]
        ...
        set count [r XEXPIRE mystream EX 100 123-0 123-1 123-2]
        assert {$count == 1}

        set result [r XTTL mystream 123-0 123-1 123-2]
        set ttl [lindex $result 0 1]
        assert {$ttl <= 100000 && $ttl > 99800}
    ...
    test {XPERSIST remove multiple items' expiration time} {
        r DEL mystream
        r XADD mystream 123-0 item 1 value a
        r XADD mystream 123-1 item 2 value b
        r XADD mystream 123-2 item 3 value c
        r XADD mystream 123-3 item 4 value d
        r XADD mystream 123-4 item 5 value e
        r XADD mystream 123-5 item 6 value f
        r XADD mystream 123-6 item 7 value g
        r XADD mystream 123-7 item 8 value h
        r XADD mystream 123-8 item 9 value i
        set result [r XTTL mystream 123-0 123-1 123-2 123-3 123-4 123-5 123-6 123-7 123-8 123-9]
        assert {[llength $result] == 9}
        set ttl1 [lindex $result 0 1]
        set ttl2 [lindex $result 1 1]
        set ttl3 [lindex $result 2 1]
        set ttl4 [lindex $result 3 1]
        set ttl5 [lindex $result 4 1]
        set ttl6 [lindex $result 5 1]
        set ttl7 [lindex $result 6 1]
        set ttl8 [lindex $result 7 1]
        set ttl9 [lindex $result 8 1]
        assert {$ttl1 == -1 && $ttl2 == -1 && $ttl3 == -1 && $ttl4 == -1 && $ttl5 == -1 && $ttl6 == -1 && $ttl7 == -1 && $ttl8 == -1 && $ttl9 == -1}

        set count [r XEXPIRE mystream EX 100 123-1 123-2 123-6 123-10]
        assert {$count == 3}

        set count [r XEXPIRE mystream PX 110000 123-0 123-4 123-11]
        assert {$count == 2}

        set result [r XTTL mystream 123-1 123-2 123-6 123-19]
        assert {[llength $result] == 3}
        set ttl1 [lindex $result 0 1]
        set ttl2 [lindex $result 1 1]
        set ttl3 [lindex $result 2 1]
        assert {$ttl1 <= 100000 && $ttl1 > 99800}
        assert {$ttl2 <= 100000 && $ttl2 > 99800}
        assert {$ttl3 <= 100000 && $ttl3 > 99800}

        set result [r XTTL mystream 123-48 123-0 123-4 123-34]
        assert {[llength $result] == 2}
        set ttl1 [lindex $result 0 1]
        set ttl2 [lindex $result 1 1]
        assert {$ttl1 <= 110000 && $ttl1 > 109800}
        assert {$ttl2 <= 110000 && $ttl2 > 109800}

        set count [r XPERSIST mystream 123-0 123-1 123-2 123-3 123-4 123-5 123-6 123-7 123-8 123-9]
        assert {$count == 5}
    ...
    test {XTTL returns time-to-live of stream item} {
        r DEL mystream
        set result [r XTTL mystream 123-0]
        assert {$result == -1}

        r XDELAY mystream SX 1 EX 1 * item 1 value a
        set result [r XTTL mystream 123-0]
        assert {[r xlen mystream] == 0}
        assert {$result == -1}
    ...
    test {XTTL returns time-to-live of multiple stream items} {
        r DEL mystream
        set result [r XTTL mystream 123-0]
        assert {$result == -1}

        r XADD mystream 123-0 item 1 value a
        r XADD mystream EX 100 123-1 item 2 value b
        r XADD mystream 123-2 item 3 value c
        r XADD mystream 123-3 item 4 value d
        r XADD mystream PX 123456 123-4 item 5 value e
        r XADD mystream 123-5 item 6 value f
        r XADD mystream 123-6 item 7 value g
        r XADD mystream 123-7 item 8 value h
        r XADD mystream 123-8 item 9 value i

        set count [r XEXPIRE mystream PX 234567 123-7 123-0 123-1 123-2 123-5 1234-89]
        assert {$count == 5}
        ...
        set ttl8 [lindex $result 7 1]
        set ttl9 [lindex $result 8 1]
        assert {$ttl4 == -1 && $ttl7 == -1 && $ttl9 == -1}
        assert {$ttl1 <= 234567 && $ttl1 > 234300}
        assert {$ttl2 <= 234567 && $ttl2 > 234300}
        assert {$ttl3 <= 234567 && $ttl3 > 234300}
        assert {$ttl5 <= 123456 && $ttl3 > 123100}
        assert {$ttl6 <= 234567 && $ttl6 > 234300}
        assert {$ttl8 <= 234567 && $ttl8 > 234300}
    }
}
...
start_server {tags {"stream needs:debug"} overrides {appendonly yes}} {
    ...
    test {XADD with MAXLEN > xlen and expiration time can propagate correctly} {
        r DEL mystream
        for {set j 0} {$j < 100} {incr j} {
            r XADD mystream EX 10 * xitem v
        }
        r XADD mystream MAXLEN 200 EX 10 * xitem v
        incr j
        assert {[r xlen mystream] == $j}
        r debug loadaof
        r XADD mystream PX 12345 * xitem v
        incr j
        assert {[r xlen mystream] == $j}
    }
}
...
start_server {tags {"stream needs:debug"} overrides {appendonly yes}} {
    ...
    test {XADD with MINID > lastid and expiration time can propagate correctly} {
        r DEL mystream
        for {set j 0} {$j < 100} {incr j} {
            set id [expr {$j+1}]
            r XADD mystream PX 123456 $id xitem v
        }
        r XADD mystream MINID 1 EX 100 * xitem v
        incr j
        assert {[r xlen mystream] == $j}
        r debug loadaof
        r XADD mystream PX 123456 * xitem v
        incr j
        assert {[r xlen mystream] == $j}
    }
}
...
start_server {tags {"stream needs:debug"} overrides {appendonly yes stream-node-max-entries 100}} {
    test {XADD with ~ MAXLEN and expiration time can propagate correctly} {
        for {set j 0} {$j < 100} {incr j} {
            r XADD mystream EX 100 * xitem v
        }
        r XADD mystream MAXLEN ~ $j EX 10 * xitem v
        incr j
        assert {[r xlen mystream] == $j}
        r config set stream-node-max-entries 1
        r debug loadaof
...
start_server {tags {"stream needs:debug"} overrides {appendonly yes stream-node-max-entries 10}} {
    ...
    test {XADD with ~ MAXLEN, LIMIT, and expiration time can propagate correctly} {
        r DEL mystream
        for {set j 0} {$j < 100} {incr j} {
            r XADD mystream PX 123456 * xitem v
        }
...
start_server {tags {"stream needs:debug"} overrides {appendonly yes stream-node-max-entries 100}} {
    test {XADD with ~ MINID and expiration time can propagate correctly} {
        for {set j 0} {$j < 100} {incr j} {
            set id [expr {$j+1}]
            r XADD mystream EX 10 $id xitem v
        }
        r XADD mystream MINID ~ $j PX 123456 * xitem v
        incr j
        assert {[r xlen mystream] == $j}
        r config set stream-node-max-entries 1
        r debug loadaof
...
start_server {tags {"stream needs:debug"} overrides {appendonly yes stream-node-max-entries 10}} {
    ...
    test {XADD with ~ MINID, LIMIT and expiration time can propagate correctly} {
        r DEL mystream
        for {set j 0} {$j < 100} {incr j} {
            set id [expr {$j+1}]
            r XADD mystream PX 123456 $id xitem v
        }
        r XADD mystream MINID ~ 55 LIMIT 30 PX 123456 * xitem v
        assert {[r xlen mystream] == 71}
        r config set stream-node-max-entries 1
        r debug loadaof
        r XADD mystream PX 123456 * xitem v
...
start_server {tags {"stream needs:debug"} overrides {appendonly yes stream-node-max-entries 10}} {
    test {XTRIM with ~ MAXLEN and expiration time can propagate correctly} {
        for {set j 0} {$j < 100} {incr j} {
            r XADD mystream EX 10 * xitem v
        }
        r XTRIM mystream MAXLEN ~ 85
...
start_server {tags {"stream"}} {
    ...
    test {XADD with expiration time can CREATE an empty stream} {
        r DEL mystream
        set id [r XADD mystream MAXLEN 0 EX 100 * a b]
        assert {[dict get [r xinfo stream mystream] length] == 0}
        assert {[r XTTL mystream $id] == {}}
    }
