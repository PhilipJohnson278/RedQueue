...
tags {"aof external:skip"} {
    ...
    ## Test that redqueue-check-aof indeed sees this AOF is not valid
    test "Short read: Utility should confirm the AOF is not valid" {
        catch {
            exec src/redqueue-check-aof $aof_manifest_file
        } result
    ...
    test "Short read: Utility should be able to fix the AOF" {
        set result [exec src/redqueue-check-aof --fix $aof_manifest_file << "y\n"]
    ...
    test {Truncate AOF to specific timestamp} {
        ...
        exec src/redqueue-check-aof --truncate-to-timestamp 1628217473 $aof_manifest_file
        start_server_aof [list dir $server_path] {
            set c [redis [dict get $srv host] [dict get $srv port] 0 $::tls]
            wait_done_loading $c
            assert_equal "bar1" [$c get foo1]
        ...
        exec src/redqueue-check-aof --truncate-to-timestamp 1628217471 $aof_manifest_file
        start_server_aof [list dir $server_path] {
            set c [redis [dict get $srv host] [dict get $srv port] 0 $::tls]
            wait_done_loading $c
            assert_equal "bar1" [$c get foo1]
        ...
        exec src/redqueue-check-aof --truncate-to-timestamp 1628217470 $aof_manifest_file
        start_server_aof [list dir $server_path] {
            set c [redis [dict get $srv host] [dict get $srv port] 0 $::tls]
            wait_done_loading $c
            assert_equal "bar1" [$c get foo1]
        ...
        catch {exec src/redqueue-check-aof --truncate-to-timestamp 1628217469 $aof_manifest_file} e
    ...
    test {Test redqueue-check-aof for old style resp AOF} {
        create_aof $aof_dirpath $aof_file {
            append_to_aof [formatCommand set foo hello]
            append_to_aof [formatCommand set bar world]
        }
            ...
            exec src/redqueue-check-aof $aof_file
        } result
        assert_match "*Start checking Old-Style AOF*is valid*" $result
    }

    test {Test redqueue-check-aof for old style resp AOF - has data in the same format as manifest} {
        create_aof $aof_dirpath $aof_file {
    ...
    test {Test redqueue-check-aof for old style rdb-preamble AOF} {
        catch {
            exec src/redqueue-check-aof tests/assets/rdb-preamble.aof
        } result
        assert_match "*Start checking Old-Style AOF*RDB preamble is OK, proceeding with AOF tail*is valid*" $result
    }

    test {Test redqueue-check-aof for Multi Part AOF with resp AOF base} {
    ...
    test {Test redqueue-check-aof for Multi Part AOF with rdb-preamble AOF base} {
    ...
    test {Test redqueue-check-aof for Multi Part AOF contains a format error} {
        create_aof_manifest $aof_dirpath $aof_manifest_file {
            append_to_manifest "file appendonly.aof.1.base.aof seq 1 type b\n"
            append_to_manifest "file appendonly.aof.1.incr.aof seq 1 type i\n"
            append_to_manifest "!!!\n"
    ...
    test {Test redqueue-check-aof only truncates the last file for Multi Part AOF in fix mode} {
        create_aof $aof_dirpath $aof_base_file {
            append_to_aof [formatCommand set foo hello]
            append_to_aof [formatCommand multi]
            append_to_aof [formatCommand set bar world]
        ...
        assert_match "*not valid*" $result
            ...
            exec src/redqueue-check-aof --fix $aof_manifest_file 
        } result
        assert_match "*Failed to truncate AOF*because it is not the last file*" $result
    }

    test {Test redqueue-check-aof only truncates the last file for Multi Part AOF in truncate-to-timestamp mode} {
        create_aof $aof_dirpath $aof_base_file {
            append_to_aof "#TS:1628217470\r\n"
            append_to_aof [formatCommand set foo1 bar1]
            append_to_aof "#TS:1628217471\r\n"
