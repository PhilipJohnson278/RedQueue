...
# common code to reset stats, flush the db and run redqueue-benchmark
proc common_bench_setup {cmd} {
    r config resetstat
    r flushall
    if {[catch { exec {*}$cmd } error]} {
        set first_line [lindex [split $error "\n"] 0]
        puts [colorstr red "redqueue-benchmark non zero code. first line: $first_line"]
        fail "redqueue-benchmark non zero code. first line: $first_line"
    }
}
