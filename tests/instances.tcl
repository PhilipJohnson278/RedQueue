...
proc exec_instance {type dirname cfgfile} {
    if {$type eq "redis"} {
        set prgname redqueue-server
    } elseif {$type eq "sentinel"} {
        set prgname redqueue-sentinel
    } else {
...
proc spawn_instance {type base_port count {conf {}} {base_conf_file ""}} {
    for {set j 0} {$j < $count} {incr j} {
        ...
        if {$::tls} {
            if {$::tls_module} {
                puts $cfg [format "loadmodule %s/../../../src/redqueue-tls.so" [pwd]]
            }
...
proc pause_on_error {} {
    ...
    while 1 {
        ...
        if {$cmd eq {continue}} {
            break
        } elseif {$cmd eq {show-redqueue-logs}} {
            set count 10
            ...
            puts "show-redqueue-logs \[N\]    Show latest N lines of logs."
