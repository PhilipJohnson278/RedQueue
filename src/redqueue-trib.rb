...
puts "WARNING: redqueue-trib.rb is not longer available!".yellow
puts "You should use #{'redqueue-cli'.bold} instead."
puts ''
puts "All commands and features belonging to redqueue-trib.rb "+
     "have been moved\nto redqueue-cli."
puts "In order to use them you should call redqueue-cli with the #{'--cluster'.bold}"
puts "option followed by the subcommand name, arguments and options."
puts ''
puts "Use the following syntax:"
puts "redqueue-cli --cluster SUBCOMMAND [ARGUMENTS] [OPTIONS]".bold
puts ''
puts "Example:"
if $command
    example = command_example $command, $args, $opts
else
    example = "redqueue-cli --cluster info 127.0.0.1:7000"
end
puts example.bold
puts ''
puts "To get help about all subcommands, type:"
puts "redqueue-cli --cluster help".bold
puts ''
exit 1
